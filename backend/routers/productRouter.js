...
productRouter.get(
	...
	expressAsyncHandler(async (req, res, next) => {
		const features = new ApiFeatures(Product.find(), req.query).search().filter().sorting().pagination();
...
productRouter.get(
	'/women',
	expressAsyncHandler(async (req, res, next) => {
		const products = await Product.find({ category: 'Women' }).sort({ createdAt: 1 }).limit(20);
...
productRouter.put(
	...
	isAdmin,
	expressAsyncHandler(async (req, res, next) => {
		const { name, title, brand, images, category, subCategory, description, price, rating, stock } = req.body;
		const product = await Product.findById(req.params.id);
		if (product) {
			product.name = name;
			product.title = title;
